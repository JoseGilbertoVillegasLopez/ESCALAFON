...
# servicio en el controlador
{
    ...
    {
        $trabajador = $usuario->getTrabajador(); # obtenemos el objeto Personal relacionado con el Usuario
        $nombreCompleto = (string) $trabajador; # convertimos el objeto Personal a cadena usando el método __toString definido en la entidad Personal
        #$puesto = $trabajador?->getInformacionLaboral()?->getPuesto()?->getNombre() ?? 'N/D'; # obtenemos el nombre del puesto o 'N/D' si no está disponible
        #$departamento = $trabajador?->getInformacionLaboral()?->getPuesto()?->getNombre() ?? 'N/D'; # obtenemos el nombre del departamento o 'N/D' si no está disponible
        ...
        $email = (new TemplatedEmail()) // creamos el correo usando plantillas Twig para el cuerpo HTML y texto plano
            ...
            ->to(new Address($trabajador->getCorreo() ?? '')) # configuramos el destinatario del correo, usando el correo del personal relacionado o del usuario
